...
async function mapsGet(baseUrl: string, params: Record<string, string>): Promise<unknown> {
...
export function buildStaticMapUrl(params: Record<string, string | undefined>): string {
...
export function buildStreetViewUrl(params: Record<string, string | undefined>): string {
  const qs = new URLSearchParams();
  for (const [k, v] of Object.entries(params)) {
    if (v !== undefined) qs.set(k, v);
  }
...
export function buildEmbedUrl(mode: string, params: Record<string, string | undefined>): string {
  const qs = new URLSearchParams();
  for (const [k, v] of Object.entries(params)) {
    if (v !== undefined) qs.set(k, v);
  }
...
export async function geocode(params: Record<string, string>): Promise<unknown> {
  return mapsGet('https://maps.googleapis.com/maps/api/geocode/json', params);
}

export async function getTimezone(params: Record<string, string>): Promise<unknown> {
...
export async function textSearch(body: unknown, fieldMask: string): Promise<unknown> {
  return mapsPost(
...
export async function nearbySearch(body: unknown, fieldMask: string): Promise<unknown> {
  return mapsPost(
...
export async function autocomplete(body: unknown): Promise<unknown> {
...
export async function getPlacePhotos(
  ...
  const details = (await getPlaceDetails(
    ...
    'photos',
    'en',
  )) as { photos?: Array<{ name: string; authorAttributions?: Array<{ displayName: string }> }> };
  ...
  const results = await Promise.all(
    photos.map(async (photo) => {
      ...
      const res = await fetch(
        `https://places.googleapis.com/v1/${photo.name}/media?${qs}`,
        { headers: { 'X-Goog-Api-Key': MAPS_API_KEY } },
      );
      const text = await res.text();
      if (!res.ok) return null;
      const data = JSON.parse(text) as { photoUri?: string };
      return {
        uri: data.photoUri ?? '',
...
export async function validateAddress(body: unknown): Promise<unknown> {
  return mapsPost('https://addressvalidation.googleapis.com/v1:validateAddress', body);
}

// ── Routes API v2 ─────────────────────────────────────────────────────────────

export async function computeRoutes(body: unknown, fieldMask: string): Promise<unknown> {
  return mapsPost(
...
export async function computeRouteMatrix(body: unknown, fieldMask: string): Promise<unknown> {
  return mapsPost(
...
export async function optimizeTours(projectId: string, body: unknown): Promise<unknown> {
  return mapsPost(
