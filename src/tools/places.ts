...
export function registerPlacesTools(server: McpServer): void {
  ...
  server.registerTool(
    'places_geocode',
    {
      ...
      inputSchema: {
        address: z.string().max(500).optional().describe('Address to geocode'),
        latlng: z.string().max(50).optional().describe("'lat,lng' string for reverse geocoding"),
        language: z.string().max(10).default('en').optional(),
        region: z.string().max(2).optional().describe('ISO 3166-1 alpha-2 region bias code'),
        components: z.string().max(500).optional().describe(
  ...
  server.registerTool(
    'places_details',
    {
      ...
      inputSchema: {
        place_id: z.string().max(500).describe("Google Place ID (e.g. from places_text_search results)"),
        fields: z.string().max(2000).default(PLACE_FIELD_MASK).optional().describe(
  ...
  server.registerTool(
    'places_text_search',
    {
      ...
      inputSchema: {
        query: z.string().max(500).describe("Search query e.g. 'romantic restaurants near Eiffel Tower'"),
        location_bias_lat: z.number().min(-90).max(90).optional().describe('Latitude to bias results toward'),
        location_bias_lng: z.number().min(-180).max(180).optional().describe('Longitude to bias results toward'),
        location_bias_radius_m: z.number().min(1).max(50000).optional().describe('Bias circle radius in metres (max 50,000)'),
        ...
        included_type: z.string().max(100).optional().describe(
  ...
  server.registerTool(
    'places_nearby_search',
    {
      ...
      inputSchema: {
        ...
        included_types: z.array(z.string().max(100)).max(50).optional().describe(
          "Place type filters e.g. ['restaurant','cafe','tourist_attraction']",
        ),
        excluded_types: z.array(z.string().max(100)).max(50).optional(),
        max_results: z.number().int().min(1).max(20).default(10).optional(),
        language_code: z.string().max(10).default('en').optional(),
  ...
  server.registerTool(
    'places_autocomplete',
    {
      ...
      inputSchema: {
        input: z.string().max(500).describe('Partial text to autocomplete'),
        location_bias_lat: z.number().min(-90).max(90).optional(),
        location_bias_lng: z.number().min(-180).max(180).optional(),
        location_bias_radius_m: z.number().min(1).max(50000).optional().describe('Bias radius in metres'),
        included_primary_types: z.array(z.string().max(100)).max(5).optional().describe(
          "Filter by primary type e.g. ['lodging','restaurant']",
        ),
        country_codes: z.array(z.string().max(2)).max(5).optional().describe('ISO 3166-1 alpha-2 country codes'),
        language_code: z.string().max(10).default('en').optional(),
  ...
  server.registerTool(
    'places_photos',
    {
      ...
      inputSchema: {
        place_id: z.string().max(500).describe('Google Place ID'),
  ...
  server.registerTool(
    'places_address_validation',
    {
      ...
      inputSchema: {
        address_lines: z.array(z.string().max(500)).min(1).max(5).describe(
          "Address lines e.g. ['1600 Amphitheatre Pkwy', 'Mountain View, CA 94043']",
        ),
        region_code: z.string().max(2).optional().describe('ISO 3166-1 alpha-2 country code'),
        locality: z.string().max(200).optional().describe('City/town'),
        administrative_area: z.string().max(200).optional().describe('State/province/region'),
